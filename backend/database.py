...
class CosmosDBService:
    ...
    async def save_mcp_config(self, config: dict):
        # config is a dict matching MCPConfig schema
    ...
    async def get_state(self, session_id: str) -> dict:
        try:
            # 1. Try fast read (assuming session_id is the partition key)
    ...
    async def delete_state(self, session_id: str):
        try:
            # First, we need the exact item to find its partition key
            state = await self.get_state(session_id)
            if state:
                # The container is partitioned by /session_id
                # Fallback to 'id' if 'session_id' is missing (for very old records)
